...
export default function Header() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 ${isDarkMode ? 'bg-white/10' : 'bg-gray-900/10'} backdrop-blur-md border-b ${isDarkMode ? 'border-white/20' : 'border-gray-800/20'}`}>
        ...
        <div className="flex items-center gap-4">
          ...
          {user ? (
            ...
            <div className="relative">
              ...
              {profileMenuOpen && (
                <div className={`absolute right-0 mt-2 w-56 ${isDarkMode ? 'bg-gray-900/95' : 'bg-white'} backdrop-blur-lg border ${isDarkMode ? 'border-blue-500/40' : 'border-gray-200'} rounded overflow-hidden shadow-xl`}>
                  <Link
                    to="/profile"
            ...
            <div className="flex gap-4">
              <Link to="/login">
                <button className={`px-6 py-2 bg-transparent border-2 ${isDarkMode ? 'border-white text-white hover:bg-white hover:text-blue-900' : 'border-black text-black hover:bg-black hover:text-white'} transition-all duration-300`}>
                  Login
                </button>
              </Link>
              <Link to="/register">
