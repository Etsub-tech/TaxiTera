  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        <h1 className={`text-4xl mb-8 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>My Profile</h1>
        ...
        <div className={`${isDarkMode ? 'bg-white/10 border-blue-300/40' : 'bg-white border-gray-200'} backdrop-blur-xl border rounded-lg p-8`}>
          ...
          <div className="flex items-center gap-6 mb-8 pb-8 border-b border-gray-600/30">
            ...
            <div>
              <h2 className={`text-3xl mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{user?.username}</h2>
              <p className={`${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>TaxiTera Member</p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <label className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Username</label>
              </div>
              <p className={`text-xl ml-8 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{user?.username}</p>
            </div>

            <div>
              <div className="flex items-center gap-3 mb-2">
                <Calendar className={`w-5 h-5 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                <label className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Member Since</label>
              </div>
              <p className={`text-xl ml-8 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>December 2025</p>
            </div>
          </div>
        </div>
      </div>
